...
During a workshop, where all students were guided to find a fitting concept for the game, the decision to create a
turn-based, "roguelike" game, where the player would, at the beginning of each run, create his own army and then proceed
to fight randomly selected opponents was made.
...
A rather "simple" concept with clear mechanics.
...
implementation was deemed to time-consuming.
The first Prototype was created using the library imgui \cite{b2}, since, at the start, it seemed like a good choice.
The library was easy to set up and use, which was important to get started and test the first equation for damage
calculation.
And while imgui~\cite{b2} was a good choice for prototyping, it had one big incompatibility: The ability to skin
the GUI objects of imgui~\cite{b2} was very limited.
Therefore, the decision was made to switch to another library at the end of the prototyping phase called
Nuklear~\cite{b3}.
...
On being finished, the first round of the combat loop would start.
...
Should the players army be the first to be exhausted, the game would end and quit.
Should on the other hand the opponents army be the first to be exhausted, the player would get the chance to select
...
Since the game was to be created without the usage of an engine, a lot of thought had to be put into the design of
those systems.
...
Since Ironbound Tactics was going to be a 2D game, the complexity and workload of the render pipeline would probably
be on the lower end, it was decided to keep with OpenGL~\cite{b4}.
...
Since Ironbound Tactics would likely have a lot of instances of the same unit be at the screen at the same time,
...
scenes.
The decision to listen to this feedback turned out to be very good.
It helped a lot to tackle the problems at that time from a different perspective, and provided a different point to
continue work, resulting in a decrease of frustration.
...
It turned out to be not to complex, as the implemented systems were simple to extend.
